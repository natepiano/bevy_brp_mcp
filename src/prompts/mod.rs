...
impl PromptRegistry {
    ...
    pub async fn list_prompts(
        ...
        let prompts: Vec<Prompt> = self.prompts
            .iter()
            .map(|handler| handler.get_prompt_info())
            .collect();
            
        Ok(ListPromptsResult {
            prompts: prompts.into_iter().map(|p| rmcp::model::Prompt {
                name: p.name,
                description: Some(p.description),
                arguments: p.arguments.map(|args| {
                    args.into_iter().map(|arg| rmcp::model::PromptArgument {
                        name: arg.name,
    ...
    pub async fn get_prompt(
        ...
        let handler = self.prompts
            .iter()
            .find(|h| h.get_prompt_info().name == name)
            .ok_or_else(|| McpError::from(rmcp::model::ErrorData::invalid_request("Prompt not found", None)))?;
            
        let response = handler.execute()?;
        
        Ok(GetPromptResult {
            description: Some(handler.get_prompt_info().description),
            messages: response.messages.into_iter().map(|msg| rmcp::model::PromptMessage {
