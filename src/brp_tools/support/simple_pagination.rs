...
pub fn paginate_if_needed(data: Value, page: usize) -> Result<PaginatedResponse, String> {
    // Quick size check
    let json_str = serde_json::to_string(&data).map_err(|e| format!("JSON serialization failed: {e}"))?;
...
fn paginate_array(arr: &[Value], page: usize) -> Result<PaginatedResponse, String> {
    ...
    Ok(PaginatedResponse::paginated(Value::Array(chunk), page, total_pages))
}

fn paginate_object(obj: serde_json::Map<String, Value>, page: usize) -> Result<PaginatedResponse, String> {
    if obj.is_empty() {
        return Ok(PaginatedResponse::complete(Value::Object(serde_json::Map::new())));
    }
    ...
    Ok(PaginatedResponse::paginated(Value::Object(chunk), page, total_pages))
}
...
mod tests {
    ...
    fn test_array_pagination() {
        ...
        let large_array: Vec<Value> = (0..100).map(|i| json!({
            "id": i,
            "data": large_string
        })).collect();
    ...
    fn test_object_pagination() {
        ...
        for i in 0..100 {
            large_obj.insert(format!("key_{i:03}"), json!({
                "id": i,
                "data": large_string
            }));
        }
